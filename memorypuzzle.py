...
FPS = 30 # Frames per second, the general speed of the programe
...
def getRandomizedBoard():
    # Ger a list of every possible shape in every possible color.
    icons = []
...
def splitIntoGroupsOf(groupSize, theList):
    ...
    for i in range(0, len(theList), groupSize):
        result.append(theList[1:1 + groupSize])
    return result
...
def drawBoxCovers(board, boxes, coverage):
    ...
    for box in boxes:
        ...
        if coverage > 0: # Only draw the box if there is a coverage
...
def coverBoxesAnimation(boxes, boxesToCover):
...
def hasWon(revealedBoxes):
    # Returns True if all of the boxes have been revealed, otherwise False
